...
var getDb = function(host, done) {
	...
	if (arguments.length === 1) {
		if (typeof arguments[0] === 'function') {
			...
			throw new Error(
				'getDb illegal invocation. User either getDb(\'hostAddr\', function(err, db) { ... }) OR getDb(function(err, db) { ... })'
			);
		}
	}
...
var replSetReconfig = function(db, config, force, done) {
	...
	db.admin().command({ replSetReconfig: config, force: force },
	                   {}, function (err) {
		if (err) {
			return done(err);
		}
...
var addNewReplSetMembers = function(db, addrToAdd,
                                    addrToRemove, shouldForce,
                                    done) {
	replSetGetConfig(db, function(err, config) {
		if (err) {
			return done(err);
