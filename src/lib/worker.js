...
var workloop = function workloop() {
	if (!hostIp || !hostIpAndPort) {
		throw new Error(
			'Must initialize with the host machine\'s addr'
		);
	}

	async.parallel([ mongo.getDb, k8s.getMongoPods ],
...
var primaryWork = function(db, pods, members, shouldForce, done) {
	...
	if (addrToAdd.length || addrToRemove.length) {
		...
		mongo.addNewReplSetMembers(
			db, addrToAdd, addrToRemove, shouldForce, done
		);
		return;
	}
